...
private:
   ...
   bool ApplyGuard(const EAContext &ctx)
   {
      ...
      else if(a == GUARD_CLOSE_BUY || a == GUARD_CLOSE_SELL)
      {
         int dir = a == GUARD_CLOSE_BUY ? BD_DIR_BUY : BD_DIR_SELL;
         BasketSide &side = a == GUARD_CLOSE_BUY ? m_basket.buy : m_basket.sell;
         eRecoveryExitCoordRequest cr = BeginFullSideClose(dir,
...
public:
   ...
   void OnTick(const EAContext &ctx, CPanel &panel)
   {
      ...
      if(exitBuy || exitSell)
      {
         DriveRecoveryExit(ctx.now); // may submit one strict cleanup command per cycle
         if(panelOpenBuy || panelOpenSell)
