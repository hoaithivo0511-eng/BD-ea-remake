...
void OnStart()
{
   ...
   //--- BD-002 async lifecycle
   Check("BD-002 no evidence stays pending", !Exec_PendingReady(PENDING_EVIDENCE_NONE));
   Check("BD-002 REQUEST accepted alone stays pending", !Exec_PendingReady(PENDING_EVIDENCE_REQUEST));
   Check("BD-002 DEAL alone stays pending", !Exec_PendingReady(PENDING_EVIDENCE_DEAL));
   Check("BD-002 ORDER_DELETE alone stays pending", !Exec_PendingReady(PENDING_EVIDENCE_ORDER_DELETE));
   Check("BD-002 resulting position state may complete before REQUEST", Exec_PendingReady(PENDING_EVIDENCE_RESULT_STATE));
   Check("BD-002 close-volume full result observed", Exec_CloseVolumeResolved(0.10, 0.00, 0.10, 0.01));
   Check("BD-002 close-volume partial target observed", Exec_CloseVolumeResolved(0.10, 0.06, 0.04, 0.01));
   Check("BD-002 close-volume insufficient change stays pending", !Exec_CloseVolumeResolved(0.10, 0.08, 0.04, 0.01));
   Check("BD-002 close-volume zero target is not completion", !Exec_CloseVolumeResolved(0.10, 0.10, 0, 0.01));
   ...
   Check("MG pct just over threshold", MG_PctDiffHit(8.17, -8, 2));
   Check("MG pct just under threshold", !MG_PctDiffHit(8.15, -8, 2));
   ...
   Check("MG daily tp %", MG_DailyTpHit(50, 0, 1000, 5));
   ...
   Check("MC apply idempotent", !MC_Apply(MC_STOP_ALL, rs, pb, ps, nc));
   ...
   Check("MC apply cycle unaffected by resume", !nc);
   ...
   CheckEq("BD-R2 Sym_PointScaleFor(_Symbol) == Sym_PointScale()", Sym_PointScaleFor(_Symbol), Sym_PointScale());
   ...
   CheckEq("BD-R1 OPEN_BUY keeps 30s", (double)Exec_HardTimeoutSec(INTENT_OPEN_BUY), BD_ASYNC_HARD_TIMEOUT_SEC);
   CheckEq("BD-R1 OPEN_SELL keeps 30s", (double)Exec_HardTimeoutSec(INTENT_OPEN_SELL), BD_ASYNC_HARD_TIMEOUT_SEC);
   CheckEq("BD-R1 CLOSE_TICKET -> 10s", (double)Exec_HardTimeoutSec(INTENT_CLOSE_TICKET), BD_ASYNC_CLOSE_HARD_TIMEOUT_SEC);
   CheckEq("BD-R1 MODIFY_SLTP -> 10s", (double)Exec_HardTimeoutSec(INTENT_MODIFY_SLTP), BD_ASYNC_CLOSE_HARD_TIMEOUT_SEC);
   CheckEq("BD-R1 NONE falls back to the short timeout", (double)Exec_HardTimeoutSec(INTENT_NONE), BD_ASYNC_CLOSE_HARD_TIMEOUT_SEC);
   Check("BD-R1 soft timeout fires before the close hard timeout", BD_ASYNC_TIMEOUT_SEC < BD_ASYNC_CLOSE_HARD_TIMEOUT_SEC);
   Check("BD-R1 close unlocks before open", BD_ASYNC_CLOSE_HARD_TIMEOUT_SEC < BD_ASYNC_HARD_TIMEOUT_SEC);
   Check("BD-R1 asymmetry holds through the function", Exec_HardTimeoutSec(INTENT_CLOSE_TICKET) < Exec_HardTimeoutSec(INTENT_OPEN_BUY));
   ...
   Check("BD-R9 hedge ON: opposite side never blocks a new series", Hedge_AllowsNewSeries(true, 5));
   Check("BD-R9 hedge OFF + opposite flat: new series allowed", Hedge_AllowsNewSeries(false, 0));
   Check("BD-R9 hedge OFF + opposite open: new series BLOCKED (v13)", !Hedge_AllowsNewSeries(false, 3));
   Check("BD-R9 impossible negative count treated as flat", Hedge_AllowsNewSeries(false, -1));
   ...
   Check("BD-R9 the old gate froze BOTH sides simultaneously", !oldBuyGate && !oldSellGate);
   Check("BD-R9 the new gate frees BOTH sides", Hedge_AllowsGridAdd(buyCount) && Hedge_AllowsGridAdd(sellCount));
   Check("BD-R9 DCA freed but a new opposite series still refused", Hedge_AllowsGridAdd(buyCount) && !Hedge_AllowsNewSeries(useHedge, sellCount));
   ...
   //--- T11: MoneyGuard account-wide flatten is a clean Recovery boundary.
